...
def train(config):
    ...
    train_set = DataLoader(BTDataset(data_path=config.hyperparameters.train_path, transform=transform), 
                            shuffle=True, 
                            batch_size=config.hyperparameters.batch_size, 
                            num_workers=config.hyperparameters.dl_workers)

    validation_set = DataLoader(BTDataset(data_path=config.hyperparameters.validate_path, transform=transform), 
                            shuffle=False, 
                            batch_size=config.hyperparameters.batch_size, 
                            num_workers=config.hyperparameters.dl_workers)
    ...
    if config.hyperparameters.gpu:
        trainer = pl.Trainer(max_epochs=config.hyperparameters.epochs, accelerator='gpu', devices=1)
    else: 
