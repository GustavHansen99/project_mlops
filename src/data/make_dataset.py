...
def main(input_filepath, output_filepath):
    ...
    for i in range(len(images_healthy)):
        if i < 0.8 * len(images_healthy):
            shutil.copy2(
                os.path.join(images_healthy[i][0], images_healthy[i][1]),
                os.path.join(output_filepath, "train", images_healthy[i][1]),
            )
        if i >= 0.8 * len(images_healthy) and i < 0.9 * len(images_healthy):
            shutil.copy2(
                os.path.join(images_healthy[i][0], images_healthy[i][1]),
                os.path.join(
                    output_filepath, "validate", images_healthy[i][1]
                ),
            )
        if i >= 0.9 * len(images_healthy):
            shutil.copy2(
                os.path.join(images_healthy[i][0], images_healthy[i][1]),
                os.path.join(output_filepath, "test", images_healthy[i][1]),
            )

    for i in range(len(images_tumor)):
        if i < 0.8 * len(images_tumor):
            shutil.copy2(
                os.path.join(images_tumor[i][0], images_tumor[i][1]),
                os.path.join(output_filepath, "train", images_tumor[i][1]),
            )
        if i >= 0.8 * len(images_tumor) and i < 0.9 * len(images_tumor):
            shutil.copy2(
                os.path.join(images_tumor[i][0], images_tumor[i][1]),
                os.path.join(output_filepath, "validate", images_tumor[i][1]),
            )
        if i >= 0.9 * len(images_tumor):
            shutil.copy2(
                os.path.join(images_tumor[i][0], images_tumor[i][1]),
                os.path.join(output_filepath, "test", images_tumor[i][1]),
            )
